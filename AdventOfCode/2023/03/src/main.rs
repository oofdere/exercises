...
fn main() {
            ...
            for i in x - 1..x + 2 {
                for j in y - 1..y + 2 {
                    ...
                    if res[i][j].is_numeric() {
                        let num = extract_num(&mut res[i], j);
    ...
    let num: i32 = nums.iter().map(|x| x.parse::<i32>().unwrap()).sum::<i32>();
...
fn extract_num(v: &mut Vec<char>, idx: usize) -> String {
    ...
    let start = v[..idx].iter_mut().rev().map_while(|mut c| {
        if c.is_numeric() {
            let d = *c;
            *c = '.';
            return Some(d);
